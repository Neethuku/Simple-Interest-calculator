...
const [validYear,setvalidYear] = useState(true)
  ...
  const validateUserInputs = (e) =>{
      ...
      // console.log(!!value.match(/^[0-9]*.?[0-9]+$/));//!! cnvrt to boolean,regular exprsn
      if(!!value.match(/^\d*\.?\d+$/)){
        //vallid pattern
        if(name==='principle'){
          setPrinciple(value)
  ...
  return (
    ...
    <div style={{width:'600px'}} className="bg-light p-5 rounded">
      ...
      <Button type="submit" style={{height:'60px',width:'50%'}} className="bg-dark" variant="contained"
      disabled={validPrinciple&&validRate&&validYear?false:true}>CALCULATE</Button>
      <Button onClick={handleReset} style={{height:'60px',width:'50%'}} className="text-dark" variant="outlined">RESET</Button>
      </Stack>
      </form>
    </div>
    </div>
